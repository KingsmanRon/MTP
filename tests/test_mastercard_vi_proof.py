...
Gated like the other database integration tests: they need
``INNTRIS_DB_INTEGRATION=1`` and a ``DATABASE_URL`` pointing at a database
migrated to head. Nothing is simulated away — the Phase-5 connector, the
same authority store, the same evaluation and consumption services, the
same policy engine.

The pinned reference implementation is required rather than optional. A
proof that silently skipped its own cryptography when the dependency was
absent would report green while proving nothing, so the pin is asserted
by a test and a mismatch fails.
"""
...
class TestUpstreamPin:
    def test_the_installed_build_is_the_one_the_connector_pins(self) -> None:
        ...
        assert provenance["matches_pin"], (
            f"connector pins {provenance['pinned_commit']} / "
            f"{provenance['pinned_package_version']}, installed "
            f"{provenance['installed_commit']} / {provenance['installed_version']}"
        )

    def test_the_proof_exercises_the_phase_5_connector(self) -> None:
        """Not a re-implementation living somewhere else."""
        from scripts.mastercard_vi.harness import ProofHarness as Harness

        module = Harness.__module__
        source = __import__("inspect").getsource(
            __import__("importlib").import_module(module)
        )
        assert "api.connectors.mastercard_vi" in source
        assert "api.authority_providers" not in source
...
class TestHeadlineCases:
    async def test_case_holds_every_assertion(
        ...
        Evidence goes to a temporary directory: CI proves the cases
        behave, and the committed pack is produced by the documented
        command rather than by a test run.
        """
...
class TestTheThreeClaims:
    ...
    async def test_an_allow_becomes_bounded_execution_authority(
        ...
        """Claim 3: bound to the exact action, the exact executor, a window."""
    ...
    async def test_every_consumption_was_preceded_by_a_re_resolution(
        self, harness: ProofHarness, tmp_path: Path, signing_key
    ) -> None:
        """Issuance-time verification alone never reaches the store."""
        for case in HEADLINE_CASES:
            await case(harness, tmp_path / "evidence", signing_key)
        assert harness.reresolutions, "no delegation was re-resolved before consuming"
...
class TestTheEvidencePackStaysGenerated:
    """The pack is build output. Committing it broke the secret gate.

    Every evidence file carries public cryptographic material —
    ``signing_key_fingerprint``, ``signature_b64``, JWK ``x``/``y`` — as
    high-entropy values next to key-shaped JSON field names. None is a
    secret, but that is exactly the shape gitleaks' ``generic-api-key``
    rule matches, and the repository scans full history, so committing
    the pack failed the gate 104 times over.

    The fix was to stop committing a build artefact whose contents change
    on every run, rather than to teach the secret scanner to ignore a
    directory. These tests keep it that way: they need no database and no
    credential, so they run everywhere.
    """
    ...
    def test_no_evidence_file_is_tracked_by_git(self) -> None:
        import subprocess

        tracked = subprocess.run(
            ["git", "ls-files", "docs/proofs/mastercard-vi/evidence"],
            cwd=self._repo_root(),
            capture_output=True,
            text=True,
            check=False,
        ...
        assert not tracked, (
            "the evidence pack is generated output and must not be committed; "
            f"tracked: {tracked}"
        )

    def test_the_evidence_directory_is_ignored(self) -> None:
        """So a proof run never leaves the working tree dirty."""
        import subprocess
        ...
        ignored = subprocess.run(
            [
                "git",
                "check-ignore",
                "docs/proofs/mastercard-vi/evidence/case-1-permitted-executed.json",
            ],
            cwd=self._repo_root(),
            capture_output=True,
            text=True,
        ...
        assert ignored.returncode == 0, (
            "docs/proofs/mastercard-vi/evidence/ must be gitignored, or a proof "
            "run dirties the tree and invites the pack being committed again"
        )
