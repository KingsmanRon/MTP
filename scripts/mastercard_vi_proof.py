...
What "offline" means
--------------------
No network call is made during the run. The VI delegation is signed by
the pinned reference implementation in-process, and the only external
system is the PostgreSQL the authority store already requires.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not provenance["matches_pin"]:
        ...
        print(
            "The installed verifiable-intent build is not the pinned one "
            f"(pinned {provenance['pinned_commit']}, installed "
            f"{provenance['installed_commit']}). Install it with:\n"
    ...
    print(
        f"VI reference implementation: {provenance['repository']}"
        f"@{provenance['pinned_commit'][:12]}"
    )
    print()
