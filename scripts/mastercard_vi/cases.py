...
Read together they demonstrate three claims:

1  Inntris verifies and consumes a real Verifiable Intent delegation as an
   authority input — through the Phase-5 connector at
   ``api/connectors/mastercard_vi``, against the pinned upstream reference
   implementation;
2  a VI-valid action is still BLOCKED by stricter current Inntris
   organisation policy (cases 2 and 3, where the chain verifies first and
   the refusal is the organisation's own);
3  an ALLOW becomes bounded execution authority tied to the exact action
   and executor, re-validated against a fresh delegation resolution before
   it may be spent (cases 4–7).
...
async def _receipts(harness: ProofHarness, result: Any, grant: Any, key: Any) -> dict[str, Any]:
    ...
    Built with ``build_evidence_chain``, which refuses to sign events
    describing different histories, then verified with the public half
    only — as an outside auditor would, and publishing the key so a reader
    can repeat the check without asking anyone for anything.
    ...
    return {
        ...
        "consumption": chain.consumption.as_public_dict() if chain.consumption else None,
...
async def _write(
    ...
    *,
    ...
    reresolution: Any = None,
    key: Any,
    notes: dict[str, Any] | None = None,
) -> None:
    document: dict[str, Any] = {
        "format": EVIDENCE_FORMAT,
        "case": {"id": outcome.case_id, "title": outcome.title, "description": description},
        "vi_upstream": upstream_provenance(),
        "vi_connector": "api/connectors/mastercard_vi",
        "vi_credential_material": credential_material(delegation),
        "vi_verification": vi_verification_view(detailed),
        "resolved_authority": resolved_authority_view(detailed.resolved),
        "action_envelope": envelope_view(envelope),
        "inntris_decision": decision_view(result),
        "grant": grant_view(grant),
        "delegated_authority_reresolution": reresolution_view(
            reresolution, detailed.resolved
        ),
...
async def case_1(harness: ProofHarness, directory: Path, key: Any) -> CaseOutcome:
    ...
    detailed = harness.resolve_detailed(claim, principal)
    outcome.expect("the VI delegation verifies", detailed.resolved.is_verified)
    outcome.expect(
        "the delegate is bound to this principal",
        detailed.resolved.delegate_binding_status.value == "bound",
    )
    outcome.expect(
        "the authority window is the whole chain's intersection",
        detailed.resolved.not_before is not None
        and detailed.resolved.not_after is not None,
    )
    ...
    outcome.expect("the decision records its policy digest", bool(result.policy_snapshot_digest))

    before = len(harness.reresolutions)
    ...
    outcome.expect(
        "the delegation was re-resolved before consumption",
        len(harness.reresolutions) == before + 1,
    )
    outcome.expect(
    ...
    await _write(
        ...
        description=(
            ...
            "organisation allowlists. The Layer 1 + Layer 2 chain verifies, "
            "current Inntris policy permits the act, and the exact executor "
            "spends the grant once — after the delegation is re-resolved and "
            "found still valid."
        ),
        ...
        grant=grant,
        attempts=[attempt],
        reresolution=harness.reresolve_evidence(claim, principal),
        key=key,
    )
    return outcome
...
async def case_2(harness: ProofHarness, directory: Path, key: Any) -> CaseOutcome:
    ...
    outcome.expect(
        "VI explicitly permits Supplier B",
        fx.SUPPLIER_B_REFERENCE in set(resolved.scope.get("allowed_payees", ())),
    )
    outcome.expect(
    ...
    outcome.expect(
        "the refusal still records the policy that refused it",
        bool(result.policy_snapshot_digest),
    )
    ...
    await _write(
        ...
        outcome,
        description=(
            "8,500.00 USD to Supplier B. Verifiable Intent permits Supplier B "
            "and the amount is inside the mandate's per-transaction range, so "
            "the delegation is valid and its supported constraints are "
        ...
        notes={
            "vi_verdict_precedes_inntris_verdict": (
                "The connector resolved and verified the delegation before any "
                "policy check ran. VI=PASS is established first, and the BLOCK "
                "that follows is the organisation's."
            ),
...
async def case_3(harness: ProofHarness, directory: Path, key: Any) -> CaseOutcome:
    ...
    await _write(
        ...
        outcome,
        description=(
            "18,000.00 USD to Supplier A — an allowlisted recipient, and "
            "2,000.00 inside the mandate's 20,000.00 per-transaction ceiling. "
            "Inntris caps a single action at 10,000.00, so the organisation "
            "refuses what the delegation permits."
        ),
        delegation=delegation,
        detailed=detailed,
        envelope=_envelope(principal, amount=amount, account=SUPPLIER_A_ACCOUNT),
        ...
        notes={
            "amount_range_is_per_transaction": (
                "mandate.payment.amount_range bounds one transaction. It is not "
                "a cumulative budget, and nothing here accumulates."
            ),
            "reason_code_provenance": (
...
async def case_4(harness: ProofHarness, directory: Path, key: Any) -> CaseOutcome:
    outcome = CaseOutcome(
        case_id="case-4-exact-action-binding",
    ...
    # inside the mandate's range. So a refusal here can only be the binding.
    outcome.expect(
    ...
    await _write(
        ...
        description=(
            ...
            "is itself within both the Inntris cap and the delegated range, so "
            "the refusal is the binding to the exact approved act, not a fresh "
            "policy denial. The failed attempt does not burn the grant, and the "
            "original action still consumes."
        ),
        delegation=delegation,
        detailed=detailed,
...
# ---------------------------------------------------------------------------
# 5. idempotent retry vs replay
# ---------------------------------------------------------------------------


async def case_5(harness: ProofHarness, directory: Path, key: Any) -> CaseOutcome:
    outcome = CaseOutcome(
        case_id="case-5-idempotent-retry-vs-replay",
        title="idempotent retry vs replay",
        vi="PASS",
        policy="ALLOW",
        grant="issued",
        consume="authorised, recovered, execution_ref_conflict",
    )
    amount = "8500.00"
    ...
    outcome.expect(
        "the first attempt spends the authority",
        first.verdict is ExecutionVerdict.EXECUTED,
    )
    outcome.expect(
    ...
    await _write(
        ...
        description=(
            ...
            "and runs the side effect once. X again is an idempotent retry: the "
            "original consumption evidence comes back and nothing runs. "
            "Execution_ref Y is a replay attempt on already-spent authority and "
            "is refused."
        ),
        delegation=delegation,
        detailed=detailed,
        ...
        notes={
            "retry_is_not_replay": (
                ...
                "Replay is a DIFFERENT execution_ref reusing consumed authority, "
                "and it is refused."
            ),
            "replay_reason_code": (
                "execution_ref_conflict is what the implemented precedence "
                "actually returns: it outranks grant_already_consumed in "
                "CONSUMPTION_REJECTION_PRECEDENCE because the caller's "
                "reference, not the grant, is what does not match. It is "
                "reported here rather than renamed."
            ),
        },
    )
    return outcome
...
async def case_6(harness: ProofHarness, directory: Path, key: Any) -> CaseOutcome:
    ...
    await _write(
        ...
        description=(
            ...
            "executor with a delegation that still re-resolves cleanly, is "
            "refused: authority is bounded in time, and holding a valid "
            "delegation for the right act is not enough once the window closed."
        ),
        delegation=delegation,
        detailed=detailed,
        envelope=_envelope(principal, amount=amount, account=SUPPLIER_A_ACCOUNT),
        ...
        notes={
            ...
            "delegation_still_valid": (
                "The mandate itself is valid for days. The grant is not: a "
                "grant never outlives its delegation, and may die long before."
            ),
        },
    )
    return outcome
...
async def case_7(harness: ProofHarness, directory: Path, key: Any) -> CaseOutcome:
    outcome = CaseOutcome(
        case_id="case-7-executor-binding",
    ...
    executor_a = harness.executor(principal.org_id, key_id="executor-a", reference="op-7a")
    # Same organisation, same scopes, the same caller-supplied reference,
    # different authenticated key. The binding derives from the key.
    executor_b = harness.executor(principal.org_id, key_id="executor-b", reference="op-7a")
    outcome.expect(
        "the two executors have different bindings",
        executor_a.binding_digest != executor_b.binding_digest,
    )
    outcome.expect(
        "they present the same caller-supplied executor_reference",
        executor_a.executor_reference == executor_b.executor_reference,
    )
    ...
    outcome.expect(
        "Executor A can still consume it", right.verdict is ExecutionVerdict.EXECUTED
    )
    grant = await harness.grant(result.grant_id)
    ...
    await _write(
        ...
        description=(
            ...
            "scopes, same caller-supplied executor_reference, different "
            "authenticated API key — presents the exact approved action and is "
            "refused. The refusal does not burn the grant, and Executor A still "
            "consumes it."
        ),
        delegation=delegation,
        detailed=detailed,
        ...
        notes={
            ...
            "executor_reference_is_not_identity": (
                "Both executors presented the same executor_reference. The "
                "binding is derived from the authenticated API key, so copying "
                "a reference proves nothing."
            ),
        },
    )
    return outcome
