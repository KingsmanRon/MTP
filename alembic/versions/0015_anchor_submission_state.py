"""Separate anchor broadcast state from anchor confirmation state.
...
def downgrade() -> None:
    raise NotImplementedError(
        "Dropping submission state would discard the only record that a "
        "transaction was broadcast but never confirmed — the exact ambiguity "
        "this revision exists to remove. Write a forward migration instead."
    )
